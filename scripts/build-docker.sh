...
docker commit `docker ps -l --format="{{.ID}}"` questomatic:latest-1
docker run --privileged -it questomatic:latest-1 /bin/sh -c 'chroot /sysroot apt install \
...
docker commit `docker ps -l --format="{{.ID}}"` questomatic:latest-2